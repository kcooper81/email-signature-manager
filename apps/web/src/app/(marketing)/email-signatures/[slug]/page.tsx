...
export function generateStaticParams() {
  return emailSignaturesPages.map((page) => ({ slug: page.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  let page = emailSignaturesPages.find((p) => p.slug === slug);
  if (!page) {
    page = await getGeneratedPage('email-signatures', slug) ?? undefined;
  }
  if (!page) return {};
