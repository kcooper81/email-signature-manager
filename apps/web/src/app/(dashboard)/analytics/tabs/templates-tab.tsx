  ...
  return (
    <div className="space-y-6">
            ...
            <div className="space-y-6">
                  ...
                  <div key={template.id} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        {index === 0 && (
                          <Badge className="bg-amber-500/10 text-amber-600 border-amber-500/20">Top</Badge>
                        )}
                      ...
                      <div className="flex items-center gap-4 text-sm">
                        ...
                        <span className={`font-medium ${
                          template.successRate >= 90 ? 'text-emerald-500' : 
                      ...
                      <div className="w-20 bg-muted rounded-full h-4 overflow-hidden">
                        <div 
                          className={`h-full rounded-full transition-all ${
                            template.successRate >= 90 ? 'bg-emerald-500' : 
              ...
              <div className="flex items-center gap-6 text-xs text-muted-foreground pt-4 border-t">
                <span className="flex items-center gap-1">
                  <span className="w-3 h-3 bg-primary rounded" /> Users Deployed
                </span>
                <span className="flex items-center gap-1">
                  <span className="w-3 h-3 bg-emerald-500 rounded" /> Success Rate
                </span>
              </div>
            </div>
          )}
      ...
      <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {data.templatePerformance.map((template, index) => (
          <Card key={template.id} className={index === 0 ? 'border-amber-500/20 bg-amber-500/10' : ''}>
